...
import "./page.css";
import signInImage1 from "@/assets/signin-image1.jpeg";
...
const JobSeekerAuthPage = () => {
    ...
    const switchMode = (m) => {
        // Preserve returnUrl parameter when switching modes
    ...
    const handleSendForgotOtp = async () => {
        try {
            ...
            const res = await axios.post(
                `${process.env.NEXT_PUBLIC_JOBSEEKER_URL}/forgot-password/send-otp`,
    ...
    const handleVerifyForgotOtp = async () => {
        try {
            ...
            const res = await axios.post(
                `${process.env.NEXT_PUBLIC_JOBSEEKER_URL}/forgot-password/verify-otp`,
                {
                    email: forgotEmail.toLowerCase(),
                    otp: forgotOtp.join(""),
                }
    ...
    const handleResetPassword = async () => {
        try {
            ...
            const res = await axios.post(
                `${process.env.NEXT_PUBLIC_JOBSEEKER_URL}/forgot-password/reset`,
                {
                    email: forgotEmail.toLowerCase(),
                    otp: forgotOtp.join(""),
                    newPassword,
    ...
    return (
        <div className="auth-full-wrapper">
            ...
            <div className="auth-left">
                <Image src={signInImage1} alt="Auth" className="auth-left-image" priority />

                <div className="auth-left-overlay">
                    <div className="overlay-bg">
                        <h1>Find Better Opportunities</h1>
                        <p>Search jobs, track applications, and build your career with confidence.</p>
                    </div>
                </div>
            </div>
                            ...
                            {!otpSent && !step3 && (
                                <>
                                    ...
                                    <div className="auth-terms-checkbox">
                                        <input
                                            type="checkbox"
                                            id="jobseeker-terms-checkbox"
                                            checked={acceptedTerms}
                                            onChange={(e) => setAcceptedTerms(e.target.checked)}
                                            className="auth-terms-checkbox-input"
                                        />
                                        <label htmlFor="jobseeker-terms-checkbox" className="auth-terms-checkbox-label">
                                            I agree to the{" "}
                                            <Link 
                                                href="/jobseeker/terms" 
                                                target="_blank"
